...
#include "abor1.intfb.h"
    ...
    IF(.NOT. SELF%MAP_DEVPTR)THEN
       CALL FIELD_ABORT ("FIELD_2RD_WRAPPER_INIT: CUDA backend needed to disable host-mapped device-pointer")
    ENDIF
    ...
    IF (.NOT. LLPERSISTENT) THEN
      IF (OML_MAX_THREADS () /= SIZE (DATA, 2)) THEN
        CALL FIELD_ABORT ('FIELD_2RD_WRAPPER_INIT: DIMENSION MISMATCH')
      ENDIF
    ENDIF

    SELF%COPY_FUNC => FIELD_2RD_COPY_FUNC (SELF%PTR, SELF%DEVPTR)
...
#include "abor1.intfb.h"
    ...
    ENDIF
    IF(.NOT. SELF%MAP_DEVPTR)THEN
       CALL FIELD_ABORT ("FIELD_2RD_OWNER_INIT: CUDA backend needed to disable host-mapped device-pointer")
    ENDIF
    ...
    SELF%COPY_FUNC => FIELD_2RD_COPY_FUNC ()
  ...
  IF (.NOT. LLCREATED) CALL FIELD_ABORT ('FIELD_2RD_COPY_OBJECT: OBJECT WAS NOT CREATED')
  ...
  IF (.NOT. LLDELETED) CALL FIELD_ABORT ('FIELD_2RD_WIPE_OBJECT: OBJECT WAS NOT DELETED')
  ...
  SUBROUTINE FIELD_2RD_OWNER_GET_HOST_DATA (SELF, MODE, PTR, QUEUE)
    ...
    IF(SELF%GET_STATUS ()==UNALLOCATED)THEN
      ...
      ENDIF
    ENDIF
    CALL SELF%FIELD_2RD_GET_HOST_DATA(MODE, PTR, QUEUE)
  ...
  SUBROUTINE FIELD_2RD_OWNER_GET_DEVICE_DATA (SELF, MODE, PTR, QUEUE)
    ...
    IF(SELF%GET_STATUS ()==UNALLOCATED)THEN
      ...
      ENDIF
    ENDIF
    CALL SELF%FIELD_2RD_GET_DEVICE_DATA(MODE, PTR, QUEUE)
