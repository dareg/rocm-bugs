...
#include "abor1.intfb.h"
    ...
    IF(.NOT. SELF%MAP_DEVPTR)THEN
       CALL FIELD_ABORT ("FIELD_4RD_WRAPPER_INIT: CUDA backend needed to disable host-mapped device-pointer")
    ENDIF
    ...
    IF (.NOT. LLPERSISTENT) THEN
      IF (OML_MAX_THREADS () /= SIZE (DATA, 4)) THEN
        CALL FIELD_ABORT ('FIELD_4RD_WRAPPER_INIT: DIMENSION MISMATCH')
      ENDIF
    ENDIF

    SELF%COPY_FUNC => FIELD_4RD_COPY_FUNC (SELF%PTR, SELF%DEVPTR)
...
#include "abor1.intfb.h"
    ...
    ENDIF
    IF(.NOT. SELF%MAP_DEVPTR)THEN
       CALL FIELD_ABORT ("FIELD_4RD_OWNER_INIT: CUDA backend needed to disable host-mapped device-pointer")
    ENDIF
    ...
    SELF%COPY_FUNC => FIELD_4RD_COPY_FUNC ()
  ...
  IF (.NOT. LLCREATED) CALL FIELD_ABORT ('FIELD_4RD_COPY_OBJECT: OBJECT WAS NOT CREATED')
  ...
  IF (.NOT. LLDELETED) CALL FIELD_ABORT ('FIELD_4RD_WIPE_OBJECT: OBJECT WAS NOT DELETED')
